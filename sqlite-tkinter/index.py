...
class Product:
    ...
    def __init__(self, window):
        ...
        # Buttons
        ttk.Button(text="Delete", command=self.deleteProduct).grid(
            row=5, column=0, sticky=W + E)
        ttk.Button(text="Edit", command=self.editProduct).grid(
        ...
        if self.selectRecord():
            ...
            Button(self.edit_wind, text="Update",
                   command=lambda: self.edit_records(
                       new_name.get(), name, new_price.get(), old_price)).grid(row=4, column=2, sticky=W + E)
    ...
    def edit_records(self, new_name, old_name, new_price, old_price):
        ...
        self.message["text"] = "Record {} updated Successfully".format(
            old_name)
        self.getProducts()
